...
const matchers: Array<{
  ...
  {
    test: (fileName) => /\bpython\b/i.test(fileName) || /python/i.test(fileName),
    metadata: {
  ...
  {
    test: (fileName) => /\b(go lang|golang|go lang)\b/i.test(fileName),
    metadata: {
  ...
  {
    test: (fileName) => /\b(introduction to html|introduction to java|introduction to c)\b/i.test(fileName),
    metadata: {
  ...
  {
    test: (fileName) => /\b(microsoft learn|achievements|learner_transcript|microsoft)\b/i.test(fileName),
    metadata: {
  ...
  {
    test: (fileName) => /\b(jpmorgan|mastercard|hsbc|aig|anz|commonwealth bank|clifford chance|datacom|pwc|bank)\b/i.test(fileName),
    metadata: {
      provider: 'Forage Simulation',
      providerTag: 'Forage',
      category: 'Cybersecurity',
      skills: PROVIDER_SKILLS.forage,
      tags: ['Cybersecurity', 'Threat Analysis', 'Incident Response', 'Enterprise']
    }
  },
  {
...
const tidyTitle = (name: string) => {
  const trimmed = name.replace(/\.(pdf)$/i, '').replace(/_/g, ' ').replace(/\s+/g, ' ').trim();
  return trimmed
    .replace(/Achievements - pendeladivyatejstudent-9021 \s*_\s*/gi, '')
    .replace(/completion certificate/gi, 'Certificate')
    .replace(/Mastercard|JPMorgan Chase & Co\.|HSBC|ANZ Australia|AIG|Commonwealth Bank|Clifford Chance|Datacom/gi, (match) => match)
...
export const inferCertificationMetadata = (fileName: string): CertificationMetadata => {
  ...
  if (/fortinet/i.test(normalized)) {
    return {
      ...
      category: 'Fortinet',
      skills: ['Network Security', 'Firewall Management', 'Threat Protection', 'Security Operations'],
      tags: ['Fortinet', 'Network Security', 'Firewall', 'Cybersecurity']
    };
  }
  ...
  return {
    ...
    category: 'Cybersecurity',
    skills: ['Security Operations', 'Technical Analysis', 'Process Automation'],
    tags: ['Certifications', 'Professional', 'Cybersecurity']
  };
};

export const makeCertificationEntry = (fileName: string, date: string) => {
  const metadata = inferCertificationMetadata(fileName);
  const title = metadata.title || tidyTitle(fileName);
  const category = CATEGORY_OVERRIDES[metadata.category] || metadata.category;
